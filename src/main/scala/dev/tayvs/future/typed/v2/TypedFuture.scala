//package dev.tayvs.future.typed.v2
//
//import dev.tayvs.future.typed.TypedFutureWrapper.TypedFutureConstructor
//
//import scala.concurrent.ExecutionContext.parasitic
//import scala.concurrent.{ExecutionContext, Future => UnsafeFuture}
//import scala.reflect.ClassTag
//import scala.util.{Failure, Success, Try}
//
//sealed abstract class TypedFuture[E <: Throwable, T](protected val fut: UnsafeFuture[T]) {
//  type ThisType[E1, T1] <: TypedFuture[E, T]
//
//  Try(12)
//  def map[T2](f: T => T2)(implicit executor: ExecutionContext): ThisType[E, T2]
//
////  def filterOr[E2 <: E : ClassTag](p: T => Boolean)(orError: E2)(implicit executor: ExecutionContext): TypedFuture[E2, T] =
////    constructor[E2, T](fut.flatMap(v => if (p(v)) UnsafeFuture.successful(v) else UnsafeFuture.failed(orError)))
//
////  protected def constructor[E2 <: E : ClassTag, T2](fut: UnsafeFuture[T2])(implicit executor: ExecutionContext): ThisType[E2, T2]
////  //  def flatMap[E2 >: E <: Throwable, T2](f: T => TypedFuture[E2, T2])(implicit executor: ExecutionContext): ThisType[E2, T2]
//}
//
//final class PureFuture[T](fut: UnsafeFuture[T]) extends TypedFuture[Nothing, T](fut) {
//  override type ThisType[_, T] = PureFuture[T]
//
//  override def map[T2](f: T => T2)(implicit executor: ExecutionContext): PureFuture[T2] =
//    new PureFuture(fut.map(f))
//
//  def flatMap[T2](f: T => PureFuture[T2])(implicit executor: ExecutionContext): PureFuture[T2] =
//    new PureFuture(fut.flatMap(f(_).fut))
//
//  def flatMap[E2 <: Throwable : ClassTag, T2](f: T => ErrorFuture[E2, T2])
//                                             (implicit executor: ExecutionContext): ErrorFuture[E2, T2] =
//    new ErrorFuture(fut.flatMap(f(_).fut))
//
////  override protected def constructor[E2 <: Nothing : ClassTag, T2](fut: UnsafeFuture[T2])
////                                                                  (implicit executor: ExecutionContext): PureFuture[T2] =
////    new PureFuture()
//}
//
//object PureFuture {
//  def apply[T](fut: UnsafeFuture[T]): PureFuture[T] = new PureFuture(fut)
//  def success[T](v: T) = new PureFuture(UnsafeFuture.successful(v))
//}
//
//final class ErrorFuture[E <: Throwable : ClassTag, T](fut: UnsafeFuture[T]) extends TypedFuture[E, T](fut) {
//  override type ThisType[E1, T] = ErrorFuture[E, T]
//
//  override def map[T2](f: T => T2)(implicit executor: ExecutionContext): ErrorFuture[E, T2] =
//    new ErrorFuture(fut.map(f))
//
//  def flatMap[T2](f: T => PureFuture[T2])(implicit executor: ExecutionContext): ErrorFuture[E, T2] =
//    new ErrorFuture(fut.flatMap(f(_).fut))
//
//  def flatMap[E2 >: E <: Throwable : ClassTag, T2](f: T => ErrorFuture[E2, T2])
//                                                  (implicit executor: ExecutionContext): ErrorFuture[E2, T2] =
//    new ErrorFuture(fut.flatMap(f(_).fut))
//
//  def recover[U >: T](r: E => U)(implicit executor: ExecutionContext): PureFuture[U] =
//    new PureFuture(fut.recover { case e: E => r(e) })
//}
//
//object ErrorFuture {
//  def apply[E <: Throwable : ClassTag, T](fut: UnsafeFuture[T]): ErrorFuture[E, T] = new ErrorFuture(fut)
//}
//
